  ...
  if (isAdmin) {
    if (Array.isArray(incoming.cases)) {
      ...
      for (const incomingCase of incoming.cases) {
        ...
        for (const skin of serverSkins) if (skin?.id) skinById.set(String(skin.id), JSON.parse(JSON.stringify(skin)));
        for (const skin of incomingSkins) if (skin?.id) skinById.set(String(skin.id), JSON.parse(JSON.stringify(skin)));
    ...
    if (Array.isArray(incoming.codes)) {
      ...
      for (const code of incoming.codes) {
        ...
        byCode.set(key, oldCode ? { ...oldCode, ...code, uses: Math.max(Number(oldCode.uses || 0), Number(code.uses || 0)) } : { ...code });
      }
    ...
    if (Array.isArray(incoming.promoCodes)) {
      ...
      for (const promo of incoming.promoCodes) {
        ...
        byCode.set(key, oldPromo ? { ...oldPromo, ...promo, uses: Math.max(Number(oldPromo.uses || 0), Number(promo.uses || 0)) } : { ...promo });
      }
