...
app.post('/api/sync', auth, async (req, res) => {
  ...
  if (isAdmin) {
    ...
    if (Array.isArray(incoming.codes)) {
      const byCode=new Map((s.codes||[]).map(x=>[String(x.code),x]));
      for(const c of incoming.codes){if(!c?.code)continue;const k=String(c.code);const old=byCode.get(k);byCode.set(k,old?{...old,...c,uses:Math.max(Number(old.uses||0),Number(c.uses||0))}:{...c});}
      s.codes=[...byCode.values()];
    }
    if (Array.isArray(incoming.promoCodes)) {
      const byCode=new Map((s.promoCodes||[]).map(x=>[String(x.code),x]));
      for(const p of incoming.promoCodes){if(!p?.code)continue;const k=String(p.code);const old=byCode.get(k);byCode.set(k,old?{...old,...p,uses:Math.max(Number(old.uses||0),Number(p.uses||0))}:{...p});}
