...
function seedState(){
 ...
 s.cases.forEach(c=>{
  ...
  c.skins.forEach((x,i)=>x.image=svg(labels[c.id][1][i]));
 });
 return s;
}
...
app.post('/api/register',async(req,res)=>{
 try{
  ...
  const passHash=await bcrypt.hash(String(pass),12);

  sql.prepare(`
   INSERT INTO users_auth(id,email,pass_hash,admin)
   VALUES(?,?,?,0)
  `).run(String(id),cleanEmail,passHash);
...
app.post('/api/redeem-code',requireAuth,(req,res)=>{
 try{
  ...
  `).get(String(req.session.userId),cleanCode);
  ...
  sql.prepare(`
   INSERT INTO code_redemptions(user_id,code)
   VALUES(?,?)
  `).run(String(req.session.userId),cleanCode);
...
app.post('/api/redeem-case-promo',requireAuth,(req,res)=>{
 try{
  ...
  const reward=Number(
   promo.reward||promo.amount||promo.value||0
  );
...
app.post('/api/sync',requireAuth,(req,res)=>{
 try{
  ...
  if(admin&&admin.admin){
   ...
   const incomingUser=incoming.user||incoming.current;
...
app.listen(PORT,()=>{
 console.log(`GOLDUP server: http://localhost:${PORT}`);
});
