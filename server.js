...
async function getState(client = pool) {
  if (client === pool && cachedState) return JSON.parse(JSON.stringify(cachedState));
  const r = await client.query('SELECT json FROM app_state WHERE id=1');
  if (!r.rows[0]) throw new Error('app_state id=1 topilmadi');
...
async function setState(s, client = pool) {
  ...
  if (client === pool) cachedState = JSON.parse(JSON.stringify(s));
}
...
app.get('/api/bootstrap', async (req, res) => {
  try {
    const s = cachedState ? JSON.parse(JSON.stringify(cachedState)) : await getState();
    res.set('Cache-Control', 'no-store');
    return res.status(200).json({ ok: true, state: publicState(s, req.session.userId || null) });
  } catch (e) {
    console.error('GOLDUP /api/bootstrap error:', e && e.stack ? e.stack : e);
    return res.status(500).json({ ok: false, error: 'Server state yuklanmadi.', detail: String(e?.message || e) });
  }
});
