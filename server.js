  ...
  if (isAdmin) {
    if (Array.isArray(incoming.cases)) {
      ...
      for (const incomingCase of incoming.cases) {
        ...
        // Merge server skins with incoming skins so stale browser state
        // can never delete keys already stored in PostgreSQL.
        ...
        const merged = { ...existing, ...incomingCase };
        delete merged.skins;
        ...
        merged.skins = [...skinById.values()];
        byId.set(id, merged);
      }
      s.cases = [...byId.values()];
    }
    // Merge codes/promo codes instead of replacing server data with stale browser data.
    if (Array.isArray(incoming.codes)) {
      ...
      for (const code of incoming.codes) {
        ...
        const oldCode = byCode.get(key);
        byCode.set(key, oldCode
          ? { ...oldCode, ...code, uses: Math.max(Number(oldCode.uses || 0), Number(code.uses || 0)) }
          : { ...code });
      }
    ...
    if (Array.isArray(incoming.promoCodes)) {
      ...
      for (const promo of incoming.promoCodes) {
        ...
        const oldPromo = byCode.get(key);
        byCode.set(key, oldPromo
          ? { ...oldPromo, ...promo, uses: Math.max(Number(oldPromo.uses || 0), Number(promo.uses || 0)) }
          : { ...promo });
      }
