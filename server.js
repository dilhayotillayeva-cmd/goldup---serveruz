  ...
  if (isAdmin) {
    if (Array.isArray(incoming.cases)) {
      ...
      for (const incomingCase of incoming.cases) {
        ...
        for (const skin of serverSkins) if (skin?.id) skinMap.set(String(skin.id), JSON.parse(JSON.stringify(skin)));
        for (const skin of incomingSkins) if (skin?.id) skinMap.set(String(skin.id), JSON.parse(JSON.stringify(skin)));
        mergedCase.skins = [...skinMap.values()];
