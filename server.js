...
app.get('/api/bootstrap', async (req, res) => {
  try { const s = await getState(); res.json({ ok: true, state: publicState(s, req.session.userId || null) }); }
  catch (e) { console.error('GOLDUP /api/bootstrap error:', e); res.status(500).json({ error: 'Server state yuklanmadi.' }); }
});
...
async function adminState(req, res) {
  const s = await getState();
  const me = userById(s, req.session.userId);
  if (!me?.admin) return res.status(403).json({error:'Admin huquqi kerak.'});
  return s;
}

app.post('/api/admin/cases', auth, async (req,res)=>{
  try {
    const s = await adminState(req,res); if (!s) return;
    const action=String(req.body?.action||''); const data=req.body?.data||{};
    if(action==='create'){
      const c={id:String(data.id||('c'+Date.now())),name:String(data.name||'').trim(),price:Number(data.price||0),image:data.image||svg(data.name||'CASE'),skins:[]};
      if(!c.name||!Number.isFinite(c.price)||c.price<=0)return res.status(400).json({error:'Key nomi yoki narxi noto‘g‘ri.'});
      if(s.cases.some(x=>String(x.id)===c.id)) c.id='c'+Date.now()+Math.floor(Math.random()*1000);
      s.cases.push(c);
    } else if(action==='update'){
      const c=s.cases.find(x=>String(x.id)===String(data.id)); if(!c)return res.status(404).json({error:'Key topilmadi.'});
      if(data.name!=null)c.name=String(data.name).trim(); if(data.price!=null)c.price=Number(data.price); if(data.image)c.image=data.image;
    } else if(action==='delete') { s.cases=s.cases.filter(x=>String(x.id)!==String(data.id)); }
  ...
  } catch(e){console.error('admin cases',e);res.status(500).json({error:'Key saqlanmadi.'});}
});

app.post('/api/admin/skins', auth, async (req,res)=>{
  try {
    const s=await adminState(req,res); if(!s)return;
    const action=String(req.body?.action||''); const data=req.body?.data||{}; const c=s.cases.find(x=>String(x.id)===String(data.caseId));
    if(!c)return res.status(404).json({error:'Key topilmadi.'}); c.skins=Array.isArray(c.skins)?c.skins:[];
    if(action==='create'){
      const sk={id:String(data.id||('s'+Date.now()+Math.floor(Math.random()*1000))),name:String(data.name||'').trim(),value:Number(data.value||0),image:data.image||svg(data.name||'SKIN'),chance:Math.max(0,Math.min(100,Number(data.chance)||0))};
      if(!sk.name||!Number.isFinite(sk.value)||sk.value<=0)return res.status(400).json({error:'Skin nomi yoki qiymati noto‘g‘ri.'}); c.skins.push(sk);
    } else if(action==='update'){
      const sk=c.skins.find(x=>String(x.id)===String(data.id)); if(!sk)return res.status(404).json({error:'Skin topilmadi.'});
      if(data.name!=null)sk.name=String(data.name).trim(); if(data.value!=null)sk.value=Number(data.value); if(data.chance!=null)sk.chance=Math.max(0,Math.min(100,Number(data.chance)||0)); if(data.image)sk.image=data.image;
    } else if(action==='delete') c.skins=c.skins.filter(x=>String(x.id)!==String(data.id));
    else return res.status(400).json({error:'Noto‘g‘ri amal.'});
    await setState(s); res.json({ok:true,state:publicState(s,req.session.userId)});
  } catch(e){console.error('admin skins',e);res.status(500).json({error:'Skin saqlanmadi.'});}
});

app.post('/api/admin/codes', auth, async (req,res)=>{
  try { const s=await adminState(req,res); if(!s)return; const action=String(req.body?.action||''); const d=req.body?.data||{}; s.codes=Array.isArray(s.codes)?s.codes:[];
    if(action==='create'){const code=String(d.code||'').trim().toUpperCase(); if(!code||s.codes.some(x=>x.code===code)||s.promoCodes?.some(x=>x.code===code))return res.status(400).json({error:'Bu kod mavjud.'}); const amount=Number(d.amount),max=Number(d.max); if(!amount||amount<1||!max||max<1)return res.status(400).json({error:'Miqdor yoki limit noto‘g‘ri.'}); s.codes.push({code,amount,uses:0,max});}
    else if(action==='delete')s.codes=s.codes.filter(x=>x.code!==String(d.code)); else return res.status(400).json({error:'Noto‘g‘ri amal.'});
    await setState(s);res.json({ok:true,state:publicState(s,req.session.userId)});
  } catch(e){console.error('admin codes',e);res.status(500).json({error:'Promokod saqlanmadi.'});}
});

app.post('/api/admin/promo', auth, async (req,res)=>{
  try { const s=await adminState(req,res); if(!s)return; const action=String(req.body?.action||''); const d=req.body?.data||{}; s.promoCodes=Array.isArray(s.promoCodes)?s.promoCodes:[];
    if(action==='create'){const code=String(d.code||'').trim().toUpperCase(); const discount=Number(d.discount),max=Number(d.max); if(!code||s.codes?.some(x=>x.code===code)||s.promoCodes.some(x=>x.code===code))return res.status(400).json({error:'Bu kod mavjud.'}); if(!d.caseId||!s.cases.some(x=>String(x.id)===String(d.caseId))||discount<1||discount>100||max<1)return res.status(400).json({error:'Promokod ma’lumotlari noto‘g‘ri.'}); s.promoCodes.push({code,caseId:String(d.caseId),discount:Math.min(100,discount),uses:0,max});}
    else if(action==='delete')s.promoCodes=s.promoCodes.filter(x=>x.code!==String(d.code)); else return res.status(400).json({error:'Noto‘g‘ri amal.'});
    await setState(s);res.json({ok:true,state:publicState(s,req.session.userId)});
  } catch(e){console.error('admin promo',e);res.status(500).json({error:'Key promokodi saqlanmadi.'});}
});
