...
const dataDir=process.env.DATA_DIR || (process.env.RENDER ? '/var/data' : path.join(__dirname,'data'));
...
function ensureAdmin(){
 ...
 const row=sql.prepare('SELECT id FROM users_auth WHERE email=?').get(email);
 if(row){
   if(String(row.id)!==id) sql.prepare('UPDATE users_auth SET id=?,pass_hash=?,admin=1 WHERE email=?').run(id,hash,email);
   else sql.prepare('UPDATE users_auth SET pass_hash=?,admin=1 WHERE email=?').run(hash,email);
 }else{
   const idRow=sql.prepare('SELECT email FROM users_auth WHERE id=?').get(id);
   if(idRow && idRow.email!==email) sql.prepare('DELETE FROM users_auth WHERE id=?').run(id);
   sql.prepare('INSERT OR REPLACE INTO users_auth(id,email,pass_hash,admin) VALUES(?,?,?,1)').run(id,email,hash);
 }
 const s=getState(); let u=userById(s,id);
 ...
 u.id=id;u.email=email;u.admin=true;u.inventory=Array.isArray(u.inventory)?u.inventory:[];u.usedCodes=Array.isArray(u.usedCodes)?u.usedCodes:[];u.usedPromoCodes=Array.isArray(u.usedPromoCodes)?u.usedPromoCodes:[];setState(s);
}
ensureAdmin();
...
// Simple API health check for Render/browser debugging.
app.get('/api/health',(req,res)=>res.json({ok:true,service:'GOLDUP',time:new Date().toISOString()}));
...
app.get('/api/bootstrap',(req,res)=>{
  try{
    ...
    console.error('GOLDUP /api/bootstrap error:',e);
    res.status(500).json({error:'Server state yuklanmadi.'});
  }
});
...
app.post('/api/sync',auth,(req,res)=>{
 ...
 if(isAdmin){
  // Never replace the whole case catalog with a stale browser copy.
  // Existing keys/skins stay on the server; matching IDs are updated and new ones are added.
  if(Array.isArray(incoming.cases)){
    const byId=new Map(s.cases.map(c=>[String(c.id),c]));
    for(const incomingCase of incoming.cases){
      if(!incomingCase?.id) continue;
      const id=String(incomingCase.id);
      const existing=byId.get(id);
      if(!existing){
        byId.set(id,JSON.parse(JSON.stringify(incomingCase)));
        continue;
      }
      const skins=Array.isArray(incomingCase.skins)?incomingCase.skins:[];
      Object.assign(existing,incomingCase);
      const oldSkins=new Map((existing.skins||[]).map(x=>[String(x.id),x]));
      for(const incomingSkin of skins){
        if(!incomingSkin?.id) continue;
        oldSkins.set(String(incomingSkin.id),JSON.parse(JSON.stringify(incomingSkin)));
      }
 ...
 if(Array.isArray(incoming.users)){for(const inc of incoming.users){if(String(inc.id)!==String(req.session.userId)&&!isAdmin)continue;const u=userById(s,inc.id);if(!u)continue;if(isAdmin){const allowed={...inc};delete allowed.pass;delete allowed.id;delete allowed.email;if(u.admin)allowed.admin=true;Object.assign(u,allowed);}else{const allowed={name:inc.name,avatar:inc.avatar,inventory:inc.inventory,usedCodes:inc.usedCodes,usedPromoCodes:inc.usedPromoCodes,caseDiscounts:inc.caseDiscounts};Object.keys(allowed).forEach(k=>{if(allowed[k]!==undefined)u[k]=allowed[k]});}}}
