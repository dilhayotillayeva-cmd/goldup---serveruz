  ...
  if (isAdmin) {
    if (Array.isArray(incoming.cases)) {
      ...
      for (const incomingCase of incoming.cases) {
        ...
        // Never replace the server's skins array with a stale browser snapshot.
        // Merge case fields and merge skins by id so every admin-added skin/key survives refreshes.
        const incomingSkins = Array.isArray(incomingCase.skins) ? incomingCase.skins : [];
        const serverSkins = Array.isArray(existing.skins) ? existing.skins : [];
        const skinById = new Map();
        for (const skin of serverSkins) {
          if (skin?.id) skinById.set(String(skin.id), JSON.parse(JSON.stringify(skin)));
        }
        for (const skin of incomingSkins) {
          if (skin?.id) skinById.set(String(skin.id), JSON.parse(JSON.stringify(skin)));
        }
        const mergedCase = { ...existing, ...incomingCase };
        mergedCase.skins = [...skinById.values()];
        byId.set(id, mergedCase);
      }
      s.cases = [...byId.values()];
    }
    if (Array.isArray(incoming.codes)) {
      const byCode = new Map((s.codes || []).map(c => [String(c.code), c]));
      for (const code of incoming.codes) {
        if (!code?.code) continue;
        const key = String(code.code);
        const old = byCode.get(key);
        byCode.set(key, old ? { ...old, ...code, uses: Math.max(Number(old.uses || 0), Number(code.uses || 0)) } : { ...code });
      }
      s.codes = [...byCode.values()];
    }
    if (Array.isArray(incoming.promoCodes)) {
      const byCode = new Map((s.promoCodes || []).map(p => [String(p.code), p]));
      for (const promo of incoming.promoCodes) {
        if (!promo?.code) continue;
        const key = String(promo.code);
        const old = byCode.get(key);
        byCode.set(key, old ? { ...old, ...promo, uses: Math.max(Number(old.uses || 0), Number(promo.uses || 0)) } : { ...promo });
      }
      s.promoCodes = [...byCode.values()];
    }
