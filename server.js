  ...
  if (isAdmin) {
    if (Array.isArray(incoming.cases)) {
      ...
      for (const incomingCase of incoming.cases) {
        ...
        delete mergedCase.skins;
        const mergedSkins = new Map();
        for (const skin of serverSkins) if (skin?.id) mergedSkins.set(String(skin.id), JSON.parse(JSON.stringify(skin)));
        for (const skin of incomingSkins) if (skin?.id) mergedSkins.set(String(skin.id), JSON.parse(JSON.stringify(skin)));
        mergedCase.skins = [...mergedSkins.values()];
